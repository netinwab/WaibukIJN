...
// Database connection - using PostgreSQL instead of Neon
...
export interface IStorage {
  ...
  updateSchoolLogo(schoolId: string, logoPath: string): Promise<School | undefined>;
  ...
  canIncreaseYearbookPrice(yearbookId: string): Promise<{ canIncrease: boolean; message?: string }>;
...
export class MemStorage implements IStorage {
  ...
  async updateSchoolLogo(schoolId: string, logoPath: string): Promise<School | undefined> {
    const school = this.schools.get(schoolId);
    if (!school) {
      return undefined;
    }
...
export class DatabaseStorage implements IStorage {
  ...
  async updateMemoryTitle(memoryId: string, title: string): Promise<Memory | undefined> {
    const result = await db.update(memories)
      .set({ title })
      .where(eq(memories.id, memoryId))
      .returning();
  ...
  async updateYearbookPrice(yearbookId: string, newPrice: string, changedBy: string): Promise<{ success: boolean; message: string; yearbook?: Yearbook }> {
    ...
    if (priceNum > oldPrice) {
      ...
      // Update lastPriceIncrease if increasing
      await db.update(yearbooks)
        .set({ 
          price: newPrice, 
          lastPriceIncrease: new Date() 
        })
        .where(eq(yearbooks.id, yearbookId));
    } else {
      // Decreasing price - no cooldown needed
      await db.update(yearbooks)
        .set({ price: newPrice })
        .where(eq(yearbooks.id, yearbookId));
    }
  ...
  async canIncreaseYearbookPrice(yearbookId: string): Promise<{ canIncrease: boolean; message?: string }> {
    const [yearbook] = await db.select().from(yearbooks).where(eq(yearbooks.id, yearbookId)).limit(1);
  ...
  async getAllPublishedYearbooks(schoolId: string): Promise<{ year: number; isPublished: boolean; price: string }[]> {
