...
import { insertUserSchema, insertSchoolSchema, insertMemorySchema, insertPublicUploadLinkSchema, insertSchoolGalleryImageSchema } from "@shared/schema";
...
const trackLoginActivity = async (
  ...
  try {
    ...
    await storage.createLoginActivity({
      ...
      browser,
      os,
      city: null, // Can be enhanced with IP geolocation service
      region: null,
      country: null,
      loginStatus,
      failureReason,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  async function getSimplePendingMemories(schoolId: string) {
    ...
    try {
      const result = await pool.query(
        'SELECT id, title, description, image_url, video_url, media_type, event_date, year, category, status, uploaded_by, created_at FROM memories WHERE school_id = $1 AND status = $2 ORDER BY created_at DESC',
        [schoolId, 'pending']
      );
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // For school accounts, validate approval status
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      // TEMPORARY: Auto-verify user during development (skip email verification)
      await storage.updateUser(user.id, {
        isEmailVerified: true,
      });
      
      /* COMMENTED OUT - Email verification code (will be re-enabled for production)
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>Thank you for creating a Waibuk account! Please verify your email address to complete your registration and access all features.</p>
            ...
            <div class="footer">
              <p>© ${new Date().getFullYear()} Waibuk. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
      ...
      await sendEmail(email.toLowerCase(), "Verify Your Waibuk Account", emailHtml);
      */
      ...
      res.json({ 
        user: userInfo, 
        message: "Account created successfully. You can now log in." // TEMPORARY: Changed message for disabled email verification
      });
    } catch (error) {
  ...
  app.get("/api/memories/school/:schoolId/pending", async (req, res) => {
    ...
    try {
      ...
      const pendingMemories = rawMemories.map(memory => ({
        ...
        videoUrl: memory.video_url ? `/public${memory.video_url}` : null,
  ...
  app.get("/api/memories/school/:schoolId/:year", async (req, res) => {
    try {
      ...
      const allMemories = await storage.getMemoriesBySchoolAndYear(schoolId, parseInt(year));
  ...
  app.get("/api/public-upload-links/:code", async (req, res) => {
    try {
      ...
      // Verify dummy code for unregistered users only
      ...
      if (!isLoggedIn) {
        // For unregistered users, check verification code (dummy: must be "1234")
        // TODO: Replace with Google reCAPTCHA when deploying to production with stable domain
  ...
  app.get("/api/public-upload-links/school/:schoolId/:year", async (req, res) => {
    try {
      ...
      const links = await storage.getPublicUploadLinksBySchoolAndYear(schoolId, parseInt(year));
      res.json(links);
    } catch (error) {
  ...
  app.post("/api/public-uploads/:code", upload.single('memoryFile'), async (req, res) => {
    try {
      ...
      // For unregistered users, verify the dummy CAPTCHA code
      if (!isLoggedIn) {
        if (verificationCode !== "1234") {
  ...
  app.post("/api/super-admin/unlock-year", requireSuperAdmin, async (req, res) => {
    try {
      ...
      if (existingPurchase) {
        ...
        // If unlocking, delete any cart items for this school and year
  ...
  app.get("/api/yearbook-access/:userId/:schoolId/:year", async (req, res) => {
    ...
    try {
      const hasAccess = await storage.checkUserYearbookAccess(userId, schoolId, parseInt(year));
      res.json({ hasAccess });
    } catch (error) {
  ...
  app.delete("/api/yearbook-codes/school/:schoolId/year/:year", async (req, res) => {
    ...
    try {
      await storage.deleteAllYearbookCodes(schoolId, parseInt(year));
