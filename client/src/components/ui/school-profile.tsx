...
export default function SchoolProfile({ school, galleryImages }: SchoolProfileProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (imageId: string) => {
      return apiRequest(`/api/schools/${school.id}/gallery/${imageId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${user?.id}`,
        },
      ...
      <Card className="bg-white/10 backdrop-blur-lg border border-white/20 shadow-2xl">
        <CardContent className="p-8">
          <div className="flex items-start gap-6">
            {/* School Logo */}
            <div className="w-24 h-24 bg-gradient-to-br from-primary/20 to-primary/10 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
              {school.logo ? (
                <img 
