  ...
  const handleDemoActivation = async () => {
    ...
    if (!customerData.email || !customerData.phone) {
      toast({
        ...
        description: "Email and phone number are required for demo activation.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (user.userType !== 'school' && (!customerData.firstName || !customerData.lastName)) {
      toast({
        title: "Name required",
        description: "Your name information is required for demo activation.",
        variant: "destructive"
      });
      return;
    }
                    ...
                    <CardContent className="p-4 space-y-3">
                      {cartItems.map((item, index) => (
                        <div key={item.id} className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-white">{formatPrice(convertPrice(parseFloat(item.price || "0")))}</span>
                        </div>
                      ))}
