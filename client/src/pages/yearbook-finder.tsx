...
import { Eye, LogOut, BookOpen, Calendar, GraduationCap, Search, MapPin, ShoppingCart, Grid3X3, List, Menu, Settings, Bell, X, Home } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
        ...
        <div className="mb-4 sm:mb-8">
          ...
          {/* School Information */}
          {selectedSchool && (
            <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-white/10 backdrop-blur-lg border border-white/20 rounded-lg shadow-2xl">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <Avatar className="h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0 border-2 border-cyan-400/50">
                  ...
                  <div key={yearData.year} className="relative">
                    <Button
                      ...
                      className={`w-full h-20 sm:h-24 flex flex-col items-center justify-center transition-all duration-200 ${
                        yearData.accessible 
                          ? "bg-green-600/20 backdrop-blur-lg border border-white/20 shadow-xl text-cyan-400 hover:bg-green-600/30 hover:text-cyan-400"
                          : yearData.canPurchase 
                          ? "bg-blue-600/20 backdrop-blur-lg border border-blue-400/30 shadow-xl text-white hover:bg-blue-600/30 hover:text-white"
                          : "bg-gray-500/10 backdrop-blur-lg border border-gray-400/20 shadow-lg text-gray-400 cursor-not-allowed opacity-50"
                      }`}
                      variant="outline"
                    ...
                    >
                      ...
                      <span className="text-xs opacity-70 mt-1 flex items-center text-center px-1">
                        {yearData.accessible ? (
                          ...
                          <><ShoppingCart className="h-2 w-2 sm:h-3 sm:w-3 mr-1 flex-shrink-0" />{formatPrice(convertPrice(yearData.price))}</>
                        ) : null}
                      </span>
                    </Button>
                    {/* Status badge */}
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 shadow-2xl rounded-lg w-full max-w-md p-6">
              <div className="text-center">
                <h3 className="text-lg font-semibold text-white mb-2">
                  {selectedSchool?.name} - Class of {selectedYearForCart.year}
                </h3>
