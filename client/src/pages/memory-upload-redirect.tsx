...
export default function MemoryUploadRedirect() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        if (user.userType === 'viewer') {
          setLocation('/viewer-dashboard?tab=memory_upload');
          return;
        }
      } catch (error) {
    ...
    // For non-registered users or other user types, redirect to upload page where they can enter the 12-character code
    setLocation('/upload');
  }, [setLocation]);
