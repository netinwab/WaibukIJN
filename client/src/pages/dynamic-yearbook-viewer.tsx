...
import { ArrowLeft, Download, ZoomIn, ZoomOut, Maximize2, Share2, BookOpen, FileText, Calendar, ChevronLeft, ChevronRight, Menu, Settings, ShoppingCart, LogOut, Home, Bell, X } from 'lucide-react';
...
export default function DynamicYearbookViewer() {
  ...
  const [match, params] = useRoute("/waibuk/:year");
  const year = params?.year || "2000";
  
  // Get school ID and preview mode from URL parameters
  ...
  const goToPage = (page: number) => {
    ...
    if (page === 0) {
      ...
      // For mobile devices or landscape orientation, show single pages
      if (isMobile || yearbook?.orientation === 'landscape') {
        ...
        // For portrait orientation, show proper two-page spreads
        setViewMode('spread');
        const contentPageIndex = Math.max(0, page - (frontCover ? 1 : 0));
  ...
  const handleBackCoverClick = () => {
    ...
    const lastContentIndex = Math.max(0, contentPages.length - 2);
  ...
  const handlePrevious = () => {
    ...
    if (viewMode === 'cover') {
      ...
      if (contentPages.length > 0) {
        const targetMode = (isMobile || yearbook?.orientation === 'landscape') ? 'single' : 'spread';
              ...
              >
                ...
                {contentPages[currentPage] && (
                  <div className="bg-white shadow-lg border border-gray-300 relative"
                       style={{ width: '442px', height: '572px', aspectRatio: '3/4' }}>
                    <div className="h-full relative">
                      <img
        ...
        return (
          <div className="flex justify-center items-center min-h-[60vh] sm:min-h-[70vh] relative">
            <div 
              ...
              style={{
                width: '1040px',  // Single page width
                height: '780px', 
                maxWidth: '95vw'
              }}
            >
        ...
        return (
          <div className="flex justify-center items-center min-h-[60vh] sm:min-h-[70vh] relative">
            <div 
              ...
              className="relative overflow-hidden border border-gray-200 shadow-2xl"
              style={{
                width: '1430px', // Wider for landscape photos
                height: '585px',  // Shorter for landscape aspect ratio
                maxWidth: '95vw'
              }}
            >
              <div 
              ...
              >
                ...
                {contentPages[currentPage] && (
                  <div className="bg-white border-r border-gray-300 relative"
                       style={{ width: '702px', height: '520px', aspectRatio: '4/3' }}> {/* Landscape aspect ratio */}
                    <div className="h-full relative">
                      <img
                        src={getSecureImageUrl(contentPages[currentPage].imageUrl)}
                        alt={contentPages[currentPage].title}
