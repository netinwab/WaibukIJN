        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 relative z-10">
          <div className="flex justify-between items-center h-14 sm:h-16">
            <div className="flex items-center min-w-0 flex-1">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                {school?.logo ? (
                  <img 
                    src={school.logo} 
                    alt={school.name} 
          ...
          <div className="flex justify-center sm:justify-start gap-2 sm:gap-3 py-4 overflow-x-auto scrollbar-hide">
            ...
            >
              ...
              {pendingMemories.length > 0 && (
                <span 
                  className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center animate-pulse shadow-lg ring-2 ring-red-200 dark:ring-red-800 transition-all duration-200 ease-out"
                ...
                <CardContent>
                  <div className="space-y-3">
                    {pendingRequests.map((request) => (
                      <div key={request.id} className="flex items-center justify-between p-4 bg-orange-500/30 backdrop-blur-lg border border-orange-400 shadow-2xl transition-all hover:text-orange-400">
                        <div>
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {filteredByFoundingYear.map((badge) => (
                              <div key={badge.id} className="flex items-center justify-between p-4 bg-green-500/30 backdrop-blur-lg border border-green-400 shadow-2xl transition-all hover:text-green-400 rounded-lg">
                                <div>
                                  <h4 className="font-medium text-green-50">{badge.fullName}</h4>
                                  ...
                                  <p className="text-xs text-green-50 mt-1">Admitted: {badge.admissionYear}</p>
                                  {badge.email && (
                                    <p className="text-xs text-green-50 mt-1">Email: {badge.email}</p>
                                  )}
                                  {badge.phoneNumber && (
                                    <p className="text-xs text-green-50 mt-1">Phone: {badge.phoneNumber}</p>
                                  )}
                                </div>
                                <div className="flex items-center space-x-2">
                                  <Button 
                            ...
                            {sortedNonGraduated.map((badge) => (
                              <div key={badge.id} className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                <div>
                                  ...
                                  {badge.email && (
                                    <p className="text-xs text-gray-500 mt-1">Email: {badge.email}</p>
                                  )}
                                  {badge.phoneNumber && (
                                    <p className="text-xs text-gray-500 mt-1">Phone: {badge.phoneNumber}</p>
                                  )}
                                </div>
                                <div className="flex items-center space-x-2">
                                  <Button 
            ...
            <Card className="bg-white/10 backdrop-blur-lg border border-white/20 shadow-xl">
              <CardContent className="p-6">
                <div className="flex items-center space-x-6">
                  {/* School Logo */}
                  <div className="w-24 h-24 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {school?.logo ? (
                      <img 
